...
function placeXOrO(squareNumber) {
    ...
    //The .some() method is used to check each element of selectredSquare array to
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //Returning true is neded for our computersTurn() function to work.
        return true;
    }
...
function drawWinLine(coordX1, CoordY1, CoordX2, CoordY2) {
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 <= y2) {
            ...
            if (y2 < y2) { y += 10; }
            //This condition cancels our animation loof if it reaches the end points.
            if (x2 >= x2 && y2 >= y2) { cancelAnimationFrame(animationLoop); }
        }
...
function resetGame() {
    //This for loop iterates thrgh each HTML square element
    for (let i = 0; i < 9; i++) {
